...
describe("GET /companies", function () {
  ...
  test("works: searching with all filters", async function() {
    const resp = await request(app)
        .get("/companies")
  ...
  test("bad request if invalid filter", async function () {
    const resp = await request(app)
        .get("/companies")
        .query({ minEmployees: 5, somekey: "somekey"});
    expect(resp.statusCode).toEqual(400);
  });
...
describe("GET /companies/:handle", function () {
  ...
  test("not found for none such company", async function () {
    const resp = await request(app).get(`/companies/none`);
