...
describe("findAll", function () {
  ...
  test("works: filter by min salary and equity", async function() {
    let jobs = await Job.findAll(
      { minSalary: 150, hasEquity: true });
    expect(jobs).toEqual([
      {
        id: testJobIds[1],
        title: "Job2",
  ...
  test("works: empty list when nothing found", async function() {
    let jobs = await Job.findAll({title: "zzzzz" });
    expect(jobs).toEqual([]);
  });
});
